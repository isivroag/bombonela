$(document).ready(function() {
    ...
    $("#formDatos").submit(function(e) {
        ...
        if (nombre.length == 0  || cel.length == 0 || tel.length == 0) {
            Swal.fire({
            ...
            $.ajax({
                ...
                type: "POST",
                dataType: "json",
                data: { nombre: nombre,  tel: tel, cel: cel, id: id, correo: correo,  opcion: opcion },
                success: function(data) {
                
                    id = data[0].id_pros;
                    nombre = data[0].nom_pros;
                    tel = data[0].tel_pros;
                    cel = data[0].cel_pros;
                    correo = data[0].correo_pros;
                    
                    if (opcion == 1) {
                        tablaVis.row.add([id, nombre,  tel, cel,correo,]).draw();
                    } else {
                        tablaVis.row(fila).data([id, nombre,  tel, cel,correo,]).draw();
                    }
                }
            });
